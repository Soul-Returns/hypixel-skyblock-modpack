...
> **Status:** 30 mods, game options and keybinds ship. Mod configs are shipped
> only where something was deliberately tuned; render distance is still open
> pending the bobby setup.
...
`keybindings.txt` is derived from `options.txt` rather than from
`/defaultoptions saveAll` — that command needs a world to run in, and
`options.txt` already holds every binding; the only format difference is a
trailing modifier field.

Three lists in the script decide what ships, and it prints every decision:

- `SHIP_CONFIGS` — mod configs to ship. Deliberately short: most mod config
  files are the mod's own generated defaults, identical on any fresh install,
  and shipping those just adds noise to every player's instance. Config files
  found but not shipped are listed on every run, so nothing is dropped silently.
- `DROP_OPTIONS` — option lines removed from the shipped `options.txt`, so
  Minecraft picks its own default. For settings we refuse to decide for someone
  else: `fullscreen`, `exclusiveFullscreen`, and for now `renderDistance` and
  `simulationDistance`.
- `OPTION_OVERRIDES` — values forced regardless of the source instance:
  `soundDevice` and the resource pack lists (machine-specific),
  `damageTiltStrength` (accessibility) and `sharePresence` (vanilla 26.2
  friend-presence, forced to `none`).
...
pack/                  the modpack itself; this directory is what Pages serves
  ...
  config/              mod configs, only where something was deliberately tuned
