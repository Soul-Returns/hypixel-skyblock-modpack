...
Hypixel is already in your server list. Keybinds, video settings and mod
configs come preconfigured — and they are yours to change. Nothing you set in
the game's own options menu is ever overwritten by an update.
...
To regenerate the defaults from a live instance: set everything up in game, run
`/defaultoptions saveAll`, then copy `config/defaultoptions/` into `pack/`.
...
pack/                  the modpack itself; this directory is what Pages serves
  ...
  config/defaultoptions/
    ...
    servers.dat        Hypixel, preadded on first launch
installer/
